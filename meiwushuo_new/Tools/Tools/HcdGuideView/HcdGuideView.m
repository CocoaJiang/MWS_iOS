...
+ (instancetype)sharedInstance {
    ...
    instance.pageControl.hidden = YES;
    return instance;
}
...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.pageIndicatorTintColor = APPMAINCOLOR;
        _pageControl.currentPageIndicatorTintColor = APPMAINCOLOR;
...
- (void)showGuideViewWithImages:(NSArray *)images {
    ...
    BOOL show = NO;//[userDefaults boolForKey:[NSString stringWithFormat:@"version_%@", version]];
    NSString * key = [NSString stringWithFormat:@"version_%@", version];
    if ([userDefaults objectForKey:key]) {
        show = [userDefaults objectForKey:key];
    }
    if (!show) {
        self.images = images;
